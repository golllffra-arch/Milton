      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <GraduationCap className="w-14 h-14 mx-auto mb-4" style={{ color: "var(--page-accent, #c9a84c)" }} />
              ...
              <Card key={item.title} className="border-0 shadow-md hover:shadow-lg transition-all rounded-xl bg-white dark:bg-gray-800">
                <CardContent className="p-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <FileText className="w-8 h-8 shrink-0" style={{ color: "var(--page-secondary, #3b82f6)" }} />
                    <div>
                      <p className="font-semibold text-sm" style={{ color: "var(--page-text, #1e3a5f)" }}>{item.title}</p>
