...
const BLOCK_TYPES: BlockTypeConfig[] = [
  ...
  {
    type: "gallery", label: "Gallery", icon: ImagePlus, color: "#f59e0b",
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button key={star} type="button" onClick={() => {
                          ...
                          <Star className={cn("h-5 w-5", (t.rating as number || 0) >= star ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground")} />
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {pages.map((p) => (
                  <SelectItem key={p.id} value={p.id}>
                    <span className="flex items-center gap-2">
                      {p.isHomepage && <Globe className="h-3 w-3 text-blue-500" />}
                      {p.title}
                    </span>
                  </SelectItem>
                ))}
        ...
        <header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b bg-background px-6 py-3">
          ...
          <div className="flex items-center gap-2">
            <div className={cn(
              ...
              selectedPage?.isPublished ? "text-green-600 border-green-300 bg-green-50 dark:bg-green-950 dark:border-green-800" : "text-amber-600 border-amber-300 bg-amber-50 dark:bg-amber-950 dark:border-amber-800",
            )}>
