...
const PROGRAMS = [
  {
    ...
    gradient: "from-[#1b3f63] to-[#fe0000]",
    icon: GraduationCap,
    badge: "Tech",
  },
  {
    ...
    gradient: "from-[#fe0000] to-[#fe0000]",
    icon: BookOpen,
    badge: "Business",
  },
  {
    ...
    gradient: "from-[#1b3f63] to-[#1b3f63]",
    icon: Shield,
    badge: "Commerce",
  },
  {
...
export default function ProgramsPage() {
  return (
    <div className="overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 text-center">
          ...
          >
            <Badge variant="outline" className="mb-6 px-4 py-2 text-sm border-white/20 text-white/80 bg-white/5 backdrop-blur-sm">
              <GraduationCap className="w-3.5 h-3.5 mr-1.5" style={{ color: "var(--page-secondary, #fe0000)" }} />
              Academic Programs
            </Badge>
          </motion.div>
          <motion.h1
                      ...
                      <Card className="h-full border-0 shadow-lg hover:shadow-2xl transition-all duration-500 bg-white dark:bg-gray-800 rounded-2xl overflow-hidden">
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="space-y-2 mb-6">
                            {program.highlights.map((h) => (
                              <div key={h} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                <div className="w-1.5 h-1.5 rounded-full bg-[#fe0000] shrink-0" />
                                <span>{h}</span>
                              </div>
                            ))}
                          </div>
                          <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <span className="inline-flex items-center text-sm font-semibold text-[#fe0000] group-hover:gap-2 transition-all whitespace-nowrap">
                              View Details
      ...
      <section className="py-20 bg-white dark:bg-gray-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeInSection className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold" style={{ color: "var(--page-text, #000000)" }}>
                ...
                <FadeInSection key={item.title}>
                  <Card className="h-full border-0 shadow-md rounded-xl" style={{ background: "var(--page-bg, #ffffff)" }}>
                    <CardContent className="p-8 text-center">
                      <div className="w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-5" style={{ background: "var(--page-primary, #1b3f63)" }}>
                        <Icon className="w-7 h-7 text-white" />
                      </div>
                      <h3 className="text-xl font-display font-bold mb-3" style={{ color: "var(--page-text, #000000)" }}>{item.title}</h3>
      ...
      {/* ─── CTA ─── */}
      <section className="relative py-24 overflow-hidden" style={{ background: "linear-gradient(135deg, var(--page-hero-from, #1b3f63), var(--page-hero-to, #1b3f63))" }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <FadeInSection>
            <GraduationCap className="w-12 h-12 mx-auto mb-6" style={{ color: "var(--page-accent, #1b3f63)" }} />
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/admissions">
                <Button size="xl" className="text-white border-0 shadow-xl group" style={{ background: "var(--page-secondary, #fe0000)" }}>
                  Apply Now
